...
$maps=Get-Content analysis/learning_maps.json -Raw | ConvertFrom-Json
...
$sheet.Save((Join-Path $pwd ('analysis/learning_map_'+$mi+'.png')),[System.Drawing.Imaging.ImageFormat]::Png);$g.Dispose();$sheet.Dispose()
}
...
$sheet.Save((Join-Path $pwd 'analysis/used_tiles.png'),[System.Drawing.Imaging.ImageFormat]::Png);$g.Dispose();$sheet.Dispose();foreach($im in $cache.Values){$im.Dispose()}
