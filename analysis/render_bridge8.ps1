...
$maps=Get-Content analysis/bridge8_maps.json -Raw | ConvertFrom-Json
...
$sheet.Save((Join-Path $pwd ('analysis/bridge8_map_'+$mi+'.png')),[System.Drawing.Imaging.ImageFormat]::Png);$g.Dispose();$sheet.Dispose()
}
...
$sheet.Save((Join-Path $pwd 'analysis/bridge8_used_tiles.png'),[System.Drawing.Imaging.ImageFormat]::Png);$g.Dispose();$sheet.Dispose();foreach($im in $cache.Values){$im.Dispose()}
