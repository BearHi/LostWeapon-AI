...
[IO.File]::WriteAllBytes((Join-Path $pwd '마법사_수작업도트_v3.LMF'),$out)
...
$canvas.Save((Join-Path $pwd 'analysis\마법사_수작업도트_v3.png'));$g.Dispose();$canvas.Dispose();foreach($t in $cache.Values){$t.Dispose()};"Saved $($rec.Count) blocks"
