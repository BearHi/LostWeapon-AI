...
$maps=Get-Content analysis/bridge3_maps.json -Raw | ConvertFrom-Json
...
$m=$maps[$mi];$sheet=New-Object System.Drawing.Bitmap 1600,1080;$g=[System.Drawing.Graphics]::FromImage($sheet);$g.Clear([System.Drawing.Color]::FromArgb(26,33,48))
for($seg=0;$seg -lt 3;$seg++){$g.DrawString(($m.name+' | x='+($seg*100)+'..'+($seg*100+99)),$font,[System.Drawing.Brushes]::White,0,[single]($seg*360));foreach($b in $m.blocks){if($b.x -ge $seg*100 -and $b.x -lt ($seg+1)*100){$x=($b.x-$seg*100)*16;$y=$seg*360+24+$b.y*16;$im=$cache[[string]$b.id];if($im){$g.DrawImage($im,[int]$x,[int]$y,16,16)}else{$g.FillRectangle([System.Drawing.Brushes]::OrangeRed,$x,$y,16,16)}}}}
$sheet.Save((Join-Path $pwd ('analysis/bridge3_map_'+$mi+'.png')),[System.Drawing.Imaging.ImageFormat]::Png);$g.Dispose();$sheet.Dispose()
}
...
$sheet.Save((Join-Path $pwd 'analysis/bridge3_used_tiles.png'),[System.Drawing.Imaging.ImageFormat]::Png);$g.Dispose();$sheet.Dispose();foreach($im in $cache.Values){$im.Dispose()}
