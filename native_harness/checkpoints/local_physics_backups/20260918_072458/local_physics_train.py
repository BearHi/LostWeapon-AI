...
class Search:
    ...
    def trial(self):
        ...
        full = None
        if best and self.rng.random() < .35:
        ...
        for family, phases in selected:
            ...
            if family == "shortcut_existing_route":
...
def main():
    ...
    if min(args.rounds, args.trials_per_map, args.minutes_per_map, args.max_steps,
           args.horizon, args.candidates, args.max_nodes, args.cache_mb) <= 0:
        ...
        try:
            for round_id in range(completed+1, end_round+1):
                for path in maps:
                    ...
                    target = targets.setdefault(path.stem, old.get("search_trials", 0)+args.trials_per_map)
