...
def fit(source,output,epochs=15,stop_file=None,deadline=None):
    ...
    incumbent_heldout=None
    incumbent_candidates=None
    from candidate_benchmark import create_benchmark,evaluate,safe_to_promote
    # Fixed held-out situations keep selection comparisons stable as training data grows.
    benchmark=create_benchmark(source,output/"candidate_benchmark.json")
    candidate_selection=evaluate(model,benchmark)
    if active.exists():
        ...
        if incumbent["schema"]==SCHEMA and incumbent["physics"]==meta["physics"]:
            ...
            incumbent_heldout=metrics(model,ts,holdout,baseline)
            incumbent_candidates=evaluate(model,benchmark)
    heldout_guard=incumbent_heldout is None or (
        heldout["risk_brier"]<=incumbent_heldout["risk_brier"]+.002 and
        heldout["endpoint_mae_pixels"]<=incumbent_heldout["endpoint_mae_pixels"]*1.02)
    selection_guard=safe_to_promote(candidate_selection,incumbent_candidates)
    promoted=qualified and best_loss<incumbent_score and heldout_guard and selection_guard
    report.update(candidate_selection=candidate_selection,incumbent_candidate_selection=incumbent_candidates,
                  heldout_regression_guard=heldout_guard,candidate_selection_guard=selection_guard)
    report["promoted"]=promoted
    if promoted:
        tmp=output/"active.tmp";torch.save(payload,tmp);tmp.replace(active)
    # Include the final gate diagnostics in latest.pt as well as the readable report.
    tmp=output/"latest.tmp";torch.save(payload,tmp);tmp.replace(latest)
    (output/"model_report.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    with (output/"learning_history.jsonl").open("a",encoding="utf-8") as history_file:
        history_file.write(json.dumps({"time":time.time(),**{k:v for k,v in report.items() if k!="history"}},ensure_ascii=False)+"\n")
    ...
    lines.extend(["",f"고정 native 후보 비교: {candidate_selection['groups']}개 상황",
                  f"모델 선택 점수 {candidate_selection['mean_utility']:.3f} / 무작위 평균 {candidate_selection['random_mean_utility']:.3f}",
                  f"모델 선택 복귀/이탈 {candidate_selection['selected_reset_rate']:.2%} / 무작위 평균 {candidate_selection['random_reset_rate']:.2%}",
...
class CandidateGuide:
    ...
    def predictions(self,context,candidates,budget=64):
        grid,obj,scalar=features(context)
    ...
    def scores(self,context,candidates,budget=64,risk_weight=6):
        p,plans=self.predictions(context,candidates,budget)
        ...
        score=np.clip(p[:,:2]@direction,-2,2)-risk_weight*risk-.05*np.clip(p[:,2],0,1)
